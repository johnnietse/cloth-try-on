...
# app = Flask(__name__)
#
# # Load folders from environment or fallbacks
# BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# UPLOAD_FOLDER = os.getenv('UPLOAD_FOLDER', os.path.join(BASE_DIR, 'static', 'uploads'))
# PROCESSED_FOLDER = os.getenv('PROCESSED_FOLDER', os.path.join(BASE_DIR, 'static', 'processed'))
# SHIRT_FOLDER = os.getenv('SHIRT_FOLDER', os.path.join(BASE_DIR, 'static', 'shirts'))
...
# def get_shirt_list():
#     try:
#         return [f for f in os.listdir(app.config['SHIRT_FOLDER']) if f.lower().endswith(('.png', '.jpg', '.jpeg'))]
#     except Exception as e:
...
#
#                 src_pts = np.float32([[0, 0], [imgShirt.shape[1], 0], [imgShirt.shape[1], imgShirt.shape[0]], [0, imgShirt.shape[0]]])
#                 dst_pts = np.float32([
...
#                 img = overlay_transparent(img, warped)
#
#             out.write(img)
#
#         cap.release()
